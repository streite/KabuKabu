...
//===   YahooAPI
//==================================================================================================
public class AlpacaJSONParser {

    //**********************************************************************************************
    //***
    //**********************************************************************************************

    public static ArrayList<Stock> getAlpacaStuff(String HTTP_Response) throws JSONException {

//        HashMap<String,Float> TickerPriceHash = new HashMap<String,Float>();

        ArrayList<Stock> allStockItems = new ArrayList<>();

        JSONObject alpacaJsonObject = new JSONObject(HTTP_Response);

        JSONArray alpacaJsonArray = alpacaJsonObject.names();

        //--------------------------------------------------------------------------------------
        //---   Loop over all JSON Objects and collect them in ArrayList
        //--------------------------------------------------------------------------------------
        for (int i = 0; i < alpacaJsonArray.length(); i++) {

            Stock currentStock = new Stock();

            String Ticker = alpacaJsonArray.getString(i);

            JSONArray foo = alpacaJsonObject.getJSONArray(Ticker);
            Object bar = foo.get(0);
            currentStock.setTicker(Ticker);

            currentStock.setPrice((float) ((JSONObject) bar).getDouble("c"));

//            currentStock.setPrice(Float.parseFloat(getString("LastTradePriceOnly", alpacaJsonArray.getJSONObject(i))));
//            currentStock.setChange(getString("Change", alpacaJsonArray.getJSONObject(i)));
//            currentStock.setPercChange(getString("PercentChange", alpacaJsonArray.getJSONObject(i)));
